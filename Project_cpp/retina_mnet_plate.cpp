...
// weight map generated via gen_wts_for_tensorrtx.py
const char* WEIGHT_MAP = "./tensorrtx_mobilenet0.25_20231101.pth";
// engine file path (serialize to and deserialize from)
const char* ENGINE_FILE = "retina_mnet.engine";
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights(WEIGHT_MAP);
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
...
int main(int argc, char** argv) {
    if (argc != 2) {
        ...
        std::cerr << "./retina_mnet_plate.exe -s   // serialize model to plan file" << std::endl;
        std::cerr << "./retina_mnet_plate.exe -d   // deserialize plan file and run inference" << std::endl;
        return -1;
    }
    ...
    {   
        ...
        for (int b = 0; b < BATCH_SIZE; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                auto xywh = get_rect_adapt_landmark(tmp, INPUT_W, INPUT_H, res[j].bbox, res[j].landmark);
                auto x1 = xywh[0];
                auto y1 = xywh[1];
                auto w_bbox = xywh[2];
                auto h_bbox = xywh[3];

                /*float points_src[4][2] = {
                {res[j].landmark[4] - x1,res[j].landmark[5] - y1},
                {res[j].landmark[6] - x1,res[j].landmark[7] - y1},
                {res[j].landmark[2] - x1,res[j].landmark[3] - y1},
                {res[j].landmark[0] - x1,res[j].landmark[1] - y1},
                };


                float points_dst[4][2] = {
                {0, 0},
                {94, 0},
                {0, 24},
                {94, 24},
