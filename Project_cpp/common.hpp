...
//static inline cv::Rect get_rect_adapt_landmark(cv::Mat& img, int input_w, int input_h, float bbox[4], float lmk[8]) { // ################### 10
static inline float* get_rect_adapt_landmark(cv::Mat & img, int input_w, int input_h, float bbox[4], float lmk[8]) { // ################### 10
    int l, r, t, b;
    float r_w = input_w / (img.cols * 1.0);
    float r_h = input_h / (img.rows * 1.0);
    if (r_h > r_w) {
...
static inline void nms(std::vector<decodeplugin::Detection>& res, float *output, float nms_thresh = 0.4) {
    ...
    for (int i = 0; i < output[0]; i++) {
        //if (output[15 * i + 1 + 4] <= 0.1) continue; // ################## 15
        if (output[13 * i + 1 + 4] <= 0.1) continue; // ################## 15
        decodeplugin::Detection det;
        //memcpy(&det, &output[15 * i + 1], sizeof(decodeplugin::Detection)); // ################## 15
        memcpy(&det, &output[13 * i + 1], sizeof(decodeplugin::Detection)); // ################## 15
        dets.push_back(det);
    }
