...
ccpd_root_path_dict = {
    'ccpd2020': 'E:/plate_recognition/CCPD2020/ccpd_green',
    'ccpd2019': 'E:/plate_recognition/CCPD2019'
}

ccpd_crop_path_dict = {
    'ccpd2020': 'E:/plate_recognition/CCPD2020/ccpd_green/crop',
    'ccpd2019': 'E:/plate_recognition/CCPD2019/crop'
}

ccpd_splits_path_dict = {
    'ccpd2020': '/media/hanxiao/data/data/LPR/CCPD2020/splits',
    'ccpd2019': 'E:/plate_recognition/CCPD2019/splits'
}
...
def get_crop_ccpd2019(dataset='ccpd2019'):
    # 限制省份数量
    ...
    for train_set in ['train','test','val']:
        ...
        with open(os.path.join(ccpd_splits_path, train_set)+'.txt') as f:
            ...
            for idx, img_name in enumerate(imgs_name):
                ...
                province_no = int(lprnum[0])
                if province_no not in provinces_count:
                    provinces_count[province_no] = 1
                else:
                    if provinces_count[province_no] < province_limit:
                        provinces_count[province_no] += 1
                    else:
                        continue
                ...
                ticks = str(int(round(time.time()*1000000)))+'.jpg'
                ...
                cv2.imencode('.jpg', lic)[1].tofile(save_filename)


def get_crop_ccpd2020(dataset='ccpd2020'):
    ...
    ccpd_splits_path = ccpd_splits_path_dict[dataset]
    for train_set in ['train','test','val']:
        province_limit = 99999
        provinces_count = {}

        imgs_path = os.path.join(ccpd_root_path, train_set)
        imgs_name = list(os.listdir(imgs_path))
        ...
        for idx, img_name in enumerate(imgs_name):
            ...
            lbl4point = img_name.split('/')[-1].rsplit('.', 1)[0].split('-')[-4]
            img_src = cv2.imread(os.path.join(imgs_path, img_name.strip()))
            lprloca = [[int(eel) for eel in el.split('&')] for el in lbl4point.split('_')]
            p0 = np.float32([lprloca[2],lprloca[1],lprloca[3],lprloca[0]])
            p1 = np.float32([(0,0),(0,80),(240,0),(240,80)])
            transform_mat=cv2.getPerspectiveTransform(p0,p1)
...
def count_province_ccpd2019(dataset='ccpd2019'):
    dignum = 5 if dataset =='ccpd2019' else 6
    provinces_count = {}
    # lp_type = '04' if dataset == 'ccpd2019' else '05'
    lp_color = 'blue' if dataset == 'ccpd2019' else 'green+s'
...
if __name__ == '__main__':
    ...
    # get_crop_ccpd2019('ccpd2019')
    get_crop_ccpd2020('ccpd2020')
    # count_province_ccpd2019()
