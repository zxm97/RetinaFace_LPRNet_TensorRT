...
def parse_args():
    ...
    parser.add_argument("--model", type=str, default='workspace/lp_model_best_20231025_CBLPRD-330k+CCPD+gen_blue+green.ckpt',help="ckeckpoint file(.ckpt) path")
...
def main(args):
    ...
    # print(task.model.state_dict().keys())
    ...
    # task.model.load_state_dict({k.replace('model.', ''): v for k, v in ckpt.items()})

    # ckpt["state_dict"] = {key.replace('model.', ''): value for key, value in ckpt["state_dict"].items()}
    # task.model.load_state_dict(ckpt["state_dict"])

    task.model.load_state_dict({key.replace('model.', ''): value for key, value in ckpt["state_dict"].items()})
