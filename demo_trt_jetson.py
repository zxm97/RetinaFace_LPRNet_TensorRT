...
parser.add_argument('--lpd_engine_file_path', default='retina_lpd/weights/mobilenet0.25_epoch_24_ccpd_blue+green+yellow+white_20231101_fp16.trt', help='trt engine file')
...
parser.add_argument('--preprocess_method', default='numpy', type=str, help='numpy, torch, opencv_cuda+cupy')
...
def get_mean_matrix_opencv_cuda(mean_bgr):
    mean_np = np.array(mean_bgr).astype(np.float32)
    mean_np = np.expand_dims(mean_np, 0)
    mean_np = mean_np.repeat(args.input_w, axis=0)
    # print(mean_np)
...
def resizeAndPad_opencv_cuda_cupy(img, size, mean_matrix,padColor=0):
    no_resize = False
    h, w = img.shape[:2]
    sh, sw = size
    ...
    if not no_resize:
        ...
        # scale and pad
        img_gpu = cv2.cuda.resize(img_gpu, (new_w, new_h), interpolation=interp)
        img_gpu = cv2.cuda.copyMakeBorder(img_gpu, pad_top, pad_bot, pad_left, pad_right, borderType=cv2.BORDER_CONSTANT, value=padColor)

        img_gpu_sub = img_gpu.clone()
        # img_gpu_sub.converTo(img_gpu_sub,cv2.CV_32FC1)
        # error: (-215:Assertion failed) !scalar.empty() || (src2.type() == src1.type() && src2.size() == src1.size())
        # print(img_gpu_sub.size())
        # print(mean_matrix.size())
        # print(img_gpu_sub.type())
        # print(mean_matrix.type())
        img_gpu_sub = cv2.cuda.subtract(img_gpu_sub, mean_matrix)

        img_raw_np = img_gpu.download()
        img_gpu_sub = img_gpu_sub.download()
    else:
        img_raw_np = img
        img_gpu = cv2.cuda_GpuMat()
        img_gpu.upload(img.astype(np.float32))
        img_gpu_sub = cv2.cuda.subtract(img_gpu, mean_matrix)
        img_gpu_sub = img_gpu_sub.download().astype(np.uint8)

    return img_raw_np, img_gpu_sub
...
def preprocess_lpd_opencv_cuda_cupy(img_path, mean_matrix):
    tic = time.time()
    img_raw_np = cv2.imread(img_path, cv2.IMREAD_COLOR)
    print('read img time', time.time()-tic)
    ...
    # img_raw = cv2.resize(img_raw, (input_w, input_h))
    tic = time.time()
    img_raw_np, img_np = resizeAndPad_opencv_cuda_cupy(img_raw_np.astype(np.float32), (args.input_h, args.input_w), mean_matrix)
    print('letterbox time', time.time()-tic)
    # cv2.imshow('asda', img_raw)

    tic = time.time()
    # img_cp = cp.asarray(img_raw_np.copy()).astype(cp.float32) - cp.asarray((104, 117, 123))
    # img_np = np.asarray(img_raw_np.copy()).astype(np.float32) - np.asarray((104, 117, 123))
    # img_np = np.asarray(img_raw_np.copy()).astype(np.float32) - mean_matrix

    print('substract time', time.time()-tic)
    # img_cp = cp.transpose(img_cp, [2, 0, 1])
    img_np = np.transpose(img_np, [2, 0, 1])
    # img_cp = cp.expand_dims(img_cp, axis=0)
    img_np = np.expand_dims(img_np, axis=0)
    # img_cp = cp.array(img_cp, dtype=cp.float32, order="C")
    img_np = np.array(img_np, dtype=np.float32, order="C") # 0.006 s
    # img_np = cp.asnumpy(img_cp)
    print('preprocess time(lpd): ', time.time()-t_pre)

    return img_raw_np.astype(np.uint8), img_np
...
def main():
    ...
    with load_engine(args.lpd_engine_file_path) as engine_lpd, engine_lpd.create_execution_context() as context_lpd:
        with load_engine(args.lpr_engine_file_path) as engine_lpr, engine_lpr.create_execution_context() as context_lpr:
            ...
            elif args.preprocess_method == 'opencv_cuda+cupy':
                mean_matrix = get_mean_matrix_opencv_cuda([104, 117, 123])
            if os.path.isdir(input_path):
                ...
                for image_file in image_files:
                    ...
                    if args.preprocess_method == 'torch':
                        ...
                        image_raw, image = preprocess_lpd_opencv_cuda_cupy(os.path.join(input_path, image_file), mean_matrix)
